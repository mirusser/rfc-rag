...
| `RfcRag__ChatModel` | *(not set)* | OpenAI-compatible model ID (e.g., `openai/gpt-4o-mini`) | Chat model for answer generation. When unset, `ask_rfc` is disabled and the server remains retrieval-only. Startup registers `ask_rfc` only when this and a valid `RfcRag__ChatProvider` are configured |
| `RfcRag__ChatProvider` | `OpenRouter` | `OpenRouter` or `Local` | Chat provider for answer generation — set explicitly with `RfcRag__ChatModel` to enable `ask_rfc` |
