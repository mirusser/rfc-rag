...
The MCP server serves two deployment profiles: retrieval-only (no chat model configured) and answer-enabled (ChatModel plus a valid ChatProvider set). When ChatModel is unset, `ask_rfc` is disabled and the server surface is limited to search and retrieval tools. When answer generation is configured, the `EvidenceBudgetChars` setting controls how much context is sent to the model for generation, bounding token spend and latency per query. This split lets the same codebase power both a lightweight retrieval server and a full Q&A server without different builds or feature flags.
...
- **Optional answering with runtime config (current)** — ChatModel plus a valid ChatProvider gates the generative tool at startup, retrieval-only deployments need no model config, and the code path is tested both ways. `EvidenceBudgetChars` provides an explicit generation budget that the Context Assembler enforces before the model call.
